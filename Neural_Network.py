...
import os
import wandb
from fun import load_data, set_optimizer, set_regularizer, data_leakage, plot_y, plot_y_hist, plot_x_scaler, set_standardizer, addtrialidentifier
...
def main(fileName, height_filtering, height_lower, height_upper, create_trial_identifier, var_thresholding, var_threshold,
          testdata_size, target, scalewith, add_prop_features, n_cross_val, decrease_trials, decrease_trials_size):
    #fileName='NN_Bachelor_Thesis/ba_trials_extra.csv'
    data = pd.read_csv(fileName, sep=',')

    if height_filtering:
        data=data[height_upper > data.loc[:, 'RightHandZ'] > height_lower] # watch out, this can lead to new all zero columns
    ...
    if decrease_trials:
        ...
        random_trials = np.random.choice(unique_trials, n_trials, replace=False)
        data = data[data['Trial_ID'].isin(random_trials)]

    # Split the data into features and target
    X = data.loc[:, 'sensor1':'active_sensors'].values
    if add_prop_features:
        X = pd.concat([X, data.loc[:, 'GunAX':'GunJZ'].values], axis=1) #adds the xsens prop features to X data
