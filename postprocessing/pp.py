...
offset = False # True: Offset markers for DNN and SGPR for no overlapping, False: No offset
...
#plot_ntrials_depth(datann, datasgpr, metric, valtestboth, model, mode, polydegree,plotfilepath, target, y_max, avg, offset, plotdeptharea, depthareastart, plottrialnumimprovement) #comment out if not needed

#################PLOT COMPARISON NN vs SGPR#################
filenamenn='Data/Post_Processing_Data/250318_Eule3_Split_NN.csv'
filenamesgpr='Data/Post_Processing_Data/250318_Eule3_Chained.csv'
#data loading
datann = pd.read_csv(filenamenn, sep=',')
datann.columns =datann.columns.str.lower().str.replace(r'[\s_]', ' ', regex=True).str.replace(r'\bval\b', 'validation', regex=True)
datasgpr = pd.read_csv(filenamesgpr, sep=',')
